    ...
    <li class="comment <?php echo $commentClass; ?>" data-id="<?php $comments->theId(); ?>" id="<?php $comments->theId(); ?>">
        <div class="comment-body">
            <div class="comment-meta">
                ...
                <div class="comment--meta">
                    <?php
                    $approve = commentApprove($comments, $comments->mail);
                    $badgeBg = !empty($approve['bgColor']) ? $approve['bgColor'] : '#999999';
                    $badgeText = '#ffffff';
                    if (preg_match('/^#([0-9a-fA-F]{6})$/', $badgeBg, $m)) {
                        $hex = $m[1];
                    ...
                    <div class="comment--author" itemprop="author">
                        ...
                        <?php endif; ?>
                        <?php if (!empty($approve['userLevel'])): ?>
                            <span class="comment-level" style="margin-left:6px;display:inline-block;padding:1px 6px;border-radius:3px;background:<?php echo $badgeBg; ?>;color:<?php echo $badgeText; ?>;font-size:12px;line-height:1.4;vertical-align:middle;"><?php echo $approve['userLevel']; ?></span>
                        <?php endif; ?>
                        <div class="comment-metadata"><?php $comments->date('Y-m-d'); ?></div>
                    </div>
                    <div class="comment--time humane--time" itemprop="datePublished" datetime="<?php $comments->date('c'); ?>">
                    </div>
                </div>
            </div>
            <div class="comment--content comment-content<?php if ($comments->parent) echo ' reply-to'; ?>" itemprop="description">
